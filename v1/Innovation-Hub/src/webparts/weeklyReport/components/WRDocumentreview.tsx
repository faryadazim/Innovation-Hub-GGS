...
const WRDocumentreview = (props: IProps) => {
  ...
  const _resReportColumns: IColumn[] = [
    ...
    },
    {
      ...
      onRender: (item: IData) => {
        if (item.Responses != "Pending") {
          return (
            <div
              className={
                ResponseStyleClass[`${item.Responses.replace(" ", "")}`]
              }
            >
              {item.Responses}
            </div>
    ...
    },
    {
      ...
      onRender: (item: IData) => {
        let status;
        if (item.Responses && item.Responses != "Pending") {
          status = "Completed";
        } else if (
          moment(item.SentDate).format("DD/MM/YYYY") !=
            moment().format("DD/MM/YYYY") &&
          item.Responses == "Pending"
        ) {
          status = "Behind Scheduled";
        } else {
          status = "Scheduled";
        }
        return (
          <div className={apStatusStyleClass[`${status.replace(" ", "")}`]}>
            {status}
          </div>
        );
      },
  ...
  const generateExcel = (): void => {
    ...
    worksheet.columns = [
      { header: "File responding", key: "ToUserName", width: 25 },
      { header: "Title", key: "Title", width: 70 },
      { header: "Send date", key: "SentDate", width: 20 },
      { header: "Response date", key: "ResponseDate", width: 20 },
      { header: "From", key: "FromUserName", width: 25 },
    ...
    arrExport.forEach((item: IData) => {
      worksheet.addRow({
        ToUserName: item.ToUserName ? item.ToUserName : "",
  ...
  const resReportErrorFunction = (error: any, functionName: string): void => {
    ...
    setResReportLoader("noLoader");
    let response = {
      ComponentName: "Weekly report - deliverable view",
...
export default WRDocumentreview;
